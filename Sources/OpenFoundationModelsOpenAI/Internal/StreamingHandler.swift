...
extension AsyncStream where Element == String {
    ...
    /// Transform each chunk with a mapping function
    internal func map<T>(_ transform: @escaping (String) -> T) -> AsyncStream<T> {
    ...
    internal func filter(_ predicate: @escaping (String) -> Bool) -> AsyncStream<String> {
    ...
    internal func debounce(for interval: TimeInterval) -> AsyncStream<String> {
        AsyncStream<String> { continuation in
            Task {
                ...
                do {
                    for try await element in self {
                        ...
                        // If enough time has passed, emit the value
                        if Date().timeIntervalSince(lastEmission) >= interval,
                    ...
                    // Emit any remaining pending value
